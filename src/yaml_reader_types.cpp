	...
	switch (node.Type()) {
		case YAML::NodeType::Scalar: {
			...
			if (scalar_value.find('-') != std::string::npos || scalar_value.find(':') != std::string::npos ||
				...
				date_t date_result;
				bool special = false;
				auto date_cast_result = Date::TryConvertDate(scalar_value.c_str(), scalar_value.length(), pos,
				                                             date_result, special, false);
		...
		case YAML::NodeType::Sequence: {
			...
			for (size_t idx = 0; idx < node.size(); idx++) {
				...
				if (first_element) {
					...
					else if (common_type.id() == LogicalTypeId::BIGINT ||
					         element_type.id() == LogicalTypeId::BIGINT) {
						common_type = LogicalType::BIGINT;
					} else if (common_type.id() == LogicalTypeId::INTEGER ||
					           element_type.id() == LogicalTypeId::INTEGER) {
			...
			for (auto &entry : struct_children) {
				if (node[entry.first]) {
					struct_values.push_back(
					    make_pair(entry.first, YAMLNodeToValue(node[entry.first], entry.second)));
				} else {

...
static void YAMLArrayElementsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
    ...
    // Output rows
    idx_t count = 0;
    idx_t max_count = std::min((idx_t)STANDARD_VECTOR_SIZE,
                                bind_data.elements.size() - bind_data.current_idx);
...
static void YAMLEachFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
    ...
    idx_t count = 0;
    idx_t max_count = std::min((idx_t)STANDARD_VECTOR_SIZE,
                                bind_data.entries.size() - bind_data.current_idx);
...
void YAMLUnnestFunctions::Register(ExtensionLoader &loader) {
    ...
    yaml_array_length_set.AddFunction(ScalarFunction({yaml_type, LogicalType::VARCHAR}, LogicalType::BIGINT, YAMLArrayLengthBinaryFunction));
    loader.RegisterFunction(yaml_array_length_set);

    // yaml_keys function
    ScalarFunctionSet yaml_keys_set("yaml_keys");
    yaml_keys_set.AddFunction(ScalarFunction({yaml_type}, LogicalType::LIST(LogicalType::VARCHAR), YAMLKeysUnaryFunction));
    yaml_keys_set.AddFunction(ScalarFunction({yaml_type, LogicalType::VARCHAR}, LogicalType::LIST(LogicalType::VARCHAR), YAMLKeysBinaryFunction));
    loader.RegisterFunction(yaml_keys_set);

    // yaml_array_elements table function
    TableFunction yaml_array_elements("yaml_array_elements", {yaml_type}, YAMLArrayElementsFunction, YAMLArrayElementsBind);
